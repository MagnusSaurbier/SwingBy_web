...
Deliberate departures from a strict physical model, and implementation quirks worth knowing about,
appear **in the maths**, not only in prose. Section [§11](#11-departures-approximations-and-oddities)
collects them.
...
- **Coordinates.** $+x$ right, $+y$ **down** (see `docs/GAME.md` §4). All 33
...
- **Numerics.** Only $+ - \times \div \sqrt{\cdot}$ appear in the physics path; $u^{3/2}$ is written
  ...
  reproducibility measure (`docs/GAME.md` §4).
...
This is intentional. **It is not a bug to be fixed
...
4. The $c = 0$ boost branch is **asymmetric in $x$ and $y$** by construction: it adds $\beta_\sigma$
   ...
   therefore always launches in $+x$. By construction, not a bug.
...
inside the function (`physics.ts:130`), kept for API symmetry. Mathematically a no-op either way.
...
`Level.objects` in the level JSON.** This follows directly from the per-body loop order above.
...
Nothing reads $\theta$ back into a force or velocity (`types.ts:71`). $\omega_i$ is deliberately
**not** randomized per load — `packages/core` reads no RNG by contract (§2) — so instead it is the
deterministic
...
landing in the $(-3, -2]$ band. Since nothing reads $\theta$, this cannot affect trajectories.
...
**Deliberate design choice** (`physics.ts:342-349`): bounds are measured as half-extents
$X_{\max}, Y_{\max}$ about a **fixed** origin $(0,0)$, not relative to anything viewport-sized — a
window-size-dependent quantity has no place in a browser-free physics module, and this module must
produce identical results in the browser and in a headless Node replay-verification run.
...
The mixed rate is deliberate: arming lives in the fixed-tick path and the decrement in the
per-frame path (`bounds.ts:14-18`). Re-arming while already counting is suppressed on purpose, so
the grace period is not extended by staying out.
...
- **No collision detection or response of any kind.** `grep -i collision packages/core/src` is
  empty. Bodies pass through one another; `Body.size`
...
- **No wall-clock time in the simulation.** Only the accumulator (§3) and the bounds/flash timers
  (§8) read $\delta$; `packages/core` reads no clock and no RNG (`docs/GAME.md` §4).
...
| #   | What                                                                                                                                                                           | Where                                       | Status                                                         |
| --- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ------------------------------------------- | -------------------------------------------------------------- |
| 1   | $\Delta t$ multiplies a per-tick² acceleration in `substepCount`; the compared quantity is not a per-substep $\Delta v$ and is $N$-independent                                 | `physics.ts:98`, `physics.ts:103`           | Deliberate; changing N invalidates all 33 hand-verified levels |
| 2   | Anchored bodies (which never move) still raise $N$ for everyone                                                                                                                | `physics.ts:77` vs `physics.ts:258`         | Deliberate (`physics.ts:64-69`)                                |
| 3   | Sequential (Gauss–Seidel) body update ⇒ result depends on level object order                                                                                                   | `physics.ts:255-291`                        | Deliberate                                                     |
| 4   | Gravity is pure inverse-square ($\mu/d^2$), unsoftened; a body at exact overlap is guarded against `NaN`, but near-overlap kicks are unbounded, and there is no reaction force | `physics.ts:129-141`                        | Owner-directed: pure inverse-square, no softening              |
| 5   | Boost/brake **rescale** speed rather than adding a vector; brake clamps at exactly $\mathbf{0}$                                                                                | `physics.ts:182-195`                        | Deliberate                                                     |
| 6   | Boost from exact rest pushes $+x$ only                                                                                                                                         | `physics.ts:186-187`                        | By construction                                                |
...
| 8   | Bounds measured as half-extents about a fixed origin $(0,0)$, not viewport-relative                                                                                            | `physics.ts:342-349`, `bounds.ts:8-13`      | Deliberate, documented in-source                               |
| 9   | `turnSpeed` deterministic, not randomized per load                                                                                                                             | `level.ts:53-71`                            | Deliberate (determinism contract)                              |
...
| 11  | Second $\mu_j = 0$ guard inside `applyGravityAcceleration`, redundant with the caller's                                                                                        | `physics.ts:130`                            | Kept for API symmetry; no-op                                   |
| 12  | Prediction's $N$ fixed from the initial state for up to 1000 ticks                                                                                                             | `physics.ts:461`                            | Deliberate                                                     |
| 13  | Prediction horizon written as two independent `if`s, not `if/else`                                                                                                             | `physics.ts:452-458`                        | Deliberate, noted in-source                                    |
| 14  | Goal test is an unswept circle test on post-tick position ⇒ tunnelling possible                                                                                                | `physics.ts:413-416`                        | Deliberate                                                     |
...
| 16  | Accumulator drops simulated time past $8\Delta t$ per frame; $\varepsilon_A = 10^{-9}$ float-noise tolerance                                                                   | `loop.ts:125`, `loop.ts:159`, `loop.ts:297` | Documented in-source                                           |
| 17  | Rocket-angle dead zone is on squared speed, so the threshold is $10^{-3}$, not $10^{-6}$                                                                                       | `physics.ts:375`                            | Deliberate                                                     |

// Level-select adornment: optional world-best per level next to your personal best.
//
// `ui/screens/levelSelect.ts`'s `levelCard()` builds a plain `string[]` `meta` array joined with
// " · " (read directly — see notes/archive/T-13-PODIUM/log.md). Rather than a DOM component that
// screen would have to import from a separate rendering pipeline, this exports a pure
// string-formatting function that slots into that existing pattern with a one-line addition. A
// small standalone badge element is also exported for contexts that do own their own DOM tree
// (e.g. a future dedicated per-level leaderboard route).
 ...
 *  by the query's metric (docs/INTERFACES.md: "never outrank verified"). Returns `null` when there's

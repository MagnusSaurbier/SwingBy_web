...
import { drawBoundsWarning, drawForceVector, drawGoalRing, drawResetFlash } from "./overlays";
...
describe("drawGoalRing", () => {
  it("pulses radius by +/-8% around goalRange*zoom via the clock", () => {
    ...
    drawGoalRing(ctxB as unknown as CanvasRenderingContext2D, 0, 0, 50, 1, Math.PI / 6); // sin(pi/2)=1 -> pulse = 1.08
...
describe("drawForceVector", () => {
  it("skips drawing when the scaled vector is below the visibility threshold", () => {
    ...
    drawForceVector(ctx as unknown as CanvasRenderingContext2D, 100, 100, 0, 0, 1);
    expect(ctx.calls.length).toBe(0);
  });

  it("draws a line + arrowhead circle for a visible force", () => {
    const { ctx } = createFakeCanvas();
    drawForceVector(ctx as unknown as CanvasRenderingContext2D, 100, 100, 0.01, 0, 1);
...
describe("drawBoundsWarning", () => {
  it("draws nothing at zero intensity", () => {
    const { ctx } = createFakeCanvas();
    drawBoundsWarning(ctx as unknown as CanvasRenderingContext2D, { width: 800, height: 600 }, 0, 0);
    expect(ctx.calls.length).toBe(0);
  });

  it("draws two strokeRects (glow + border) at nonzero intensity", () => {
    const { ctx } = createFakeCanvas();
    drawBoundsWarning(ctx as unknown as CanvasRenderingContext2D, { width: 800, height: 600 }, 0.6, 0);
...
describe("drawResetFlash", () => {
  it("draws nothing at zero", () => {
    const { ctx } = createFakeCanvas();
    drawResetFlash(ctx as unknown as CanvasRenderingContext2D, { width: 800, height: 600 }, 0);
    expect(ctx.calls.length).toBe(0);
  });

  it("draws a full-viewport fillRect scaled by flash", () => {
    const { ctx } = createFakeCanvas();
    drawResetFlash(ctx as unknown as CanvasRenderingContext2D, { width: 800, height: 600 }, 1);

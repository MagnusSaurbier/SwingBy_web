...
describe("starfield", () => {
  ...
  it("moves stars opposite the player offset, scaled by each layer's parallax", () => {
    ...
    drawStarfield(
      ctxStill as unknown as CanvasRenderingContext2D,
      layers,
      viewport,
      0,
      0,
    ...
    drawStarfield(
      ctxMoved as unknown as CanvasRenderingContext2D,
      layers,
      viewport,
      100,
      0,
    ...
    const anyShifted = arcsStill.some(
      (c, i) => c.args[0] !== arcsMoved[i]!.args[0],
    );

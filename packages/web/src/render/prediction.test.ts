...
describe("prediction", () => {
  it("draws one fill() call for the player track and one for all planet tracks combined", () => {
    ...
    drawPrediction(ctx as unknown as CanvasRenderingContext2D, prediction, 0, 0, 1, { width: 800, height: 600 });
  ...
  it("player dot radius grows with zoom, floored at 1.25", () => {
    ...
    drawPrediction(ctxLowZoom as unknown as CanvasRenderingContext2D, prediction, 0, 0, 0.01, { width: 800, height: 600 });
    const { ctx: ctxHighZoom } = createFakeCanvas();
    drawPrediction(ctxHighZoom as unknown as CanvasRenderingContext2D, prediction, 0, 0, 5, { width: 800, height: 600 });

    const radiusLow = ctxLowZoom.calls.find((c) => c.method === "arc")!.args[2] as number;
    const radiusHigh = ctxHighZoom.calls.find((c) => c.method === "arc")!.args[2] as number;
